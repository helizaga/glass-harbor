// @title Filter Memory Carousel Variant 1
// @genre french house study / disco-touch loop house
// @bpm 122
// @details Pasteable song file for Strudel web. Run `glass-harbor song serve` before pasting. Variant scaffold 1 explores earlier filter lift and denser loop motion.
...
const bassLift = note("<[d2 a1 d2 a1] [c2 g1 c2 g1] [bb1 f1 bb1 f1] [a1 e1 a1 e1]>")
  .slow(4)
  .sound("triangle,sawtooth")
  .lpf(sine.range(620, 1180).slow(8))
  .attack(0.01)
  .decay(0.12)
  .sustain(0.34)
  .release(0.08)
...
const padLift = padDust
  .lpf(sine.range(1180, 2800).slow(8))
  .gain(0.18)
...
const hookRobot = note("<[d5 ~ c5 ~ a4 ~ c5 ~] [c5 ~ bb4 ~ g4 ~ bb4 ~] [bb4 ~ a4 ~ f4 ~ a4 ~] [a4 ~ g4 ~ e4 ~ g4 ~]>")
  ...
  .vowel("<i e a o>")
  .lpf(sine.range(1100, 2600).slow(8))
  .attack(0.03)
  .decay(0.12)
  .sustain(0.18)
  .release(0.08)
...
stack(
  ...
  arrange(
    ...
    [8, openHat.gain(0.2)],
    [8, silence],
    [16, openHat.gain(0.24)],
    [8, silence]
  ),

  arrange(
    [8, percSnap.gain(0.08)],
    [16, percSnap.gain(0.22)],
    [8, percSnap.gain(0.28)],
    [8, silence],
    [16, percSnap.gain(0.26)],
    [8, silence]
  ),
  ...
  arrange(
    [8, silence],
    [16, hookRobot.gain(0.08)],
    [8, hookRobot.gain(0.14)],
    [8, robotAnswer.gain(0.06)],
    [16, hookRobot.gain(1.08)],
    [8, silence]
  ),
