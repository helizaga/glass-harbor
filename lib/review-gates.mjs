...
export function buildRunVerdict({ slug, runDir, critique, baselineRunDir, baselineCritique }) {
  ...
  if (critique?.gate === 'blocked') {
    ...
    verdict = 'baseline';
    recommendedNextAction = critique?.gate === 'pass' ? 'review_gate' : 'revise';
    approvalRequired = critique?.gate === 'pass';
    summary =
      critique?.gate === 'pass'
  ...
  } else if (targetImproved || weightedDelta >= WEIGHTED_IMPROVEMENT_MIN || critique?.gate === 'pass') {
    verdict = 'improved';
