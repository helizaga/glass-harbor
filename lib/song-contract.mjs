...
export function findLatestRunDir(slug) {
  const songRunRoot = join(runsRoot, sanitizeSlug(slug));
  if (!existsSync(songRunRoot)) {
    return null;
  }

  const runs = readdirSync(songRunRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort((left, right) => right.localeCompare(left));

  return runs[0] ? join(songRunRoot, runs[0]) : null;
}
...
export function normalizeRuntimeErrors(consoleErrors = []) {
  return summarizeRuntimeErrors(consoleErrors).map((message) => ({
    code: 'runtime_error',
